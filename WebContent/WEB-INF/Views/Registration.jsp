...
<body>
	<form class="box login" action="SaveUser.do" method="post">
      ...
      if(this.checkValidity()) form.emailId.pattern = this.value;"
				tabindex="1" name="emailId" id="emailId" required /> <label>FirstName</label>
      ...
      if(this.checkValidity()) form.firstName.pattern = this.value;"
				name="firstName" id="firstName" required /> <label>LastName</label>
			<input type="text" tabindex="1" pattern="^[a-zA-Z]{3,20}$"
				onChange="this.setCustomValidity(this.validity.patternMismatch ? 'please enter the alphabets from 3 to 20' : '');
      if(this.checkValidity()) form.lastName.pattern = this.value;"
				name="lastName" id="lastName" required /> <label>Password<i></i>
			</label> <input type="password" tabindex="1" pattern="^[a-zA-Z]{3,20}$"
				onChange="this.setCustomValidity(this.validity.patternMismatch ? 'please enter the alphabets from 3 to 20' : '');
      if(this.checkValidity()) form.password.pattern = this.value;"
				name="password" id="password" required /> <label>ConfirmPassword</label>
			<input type="password" tabindex="2" name="confirmPassword"
				id="confirmPassword" onblur="matchPassword()" required /> <label>City
      ...
      if(this.checkValidity()) form.city.pattern = this.value;"
		...
		<footer> <input type="submit" class="btnRegistration"
			value="Sign Up" tabindex="4"> <%--  <fieldset>
      <tr>
      <td>
       ${registerationError}      
      </td>
